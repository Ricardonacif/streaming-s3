...
function StreamingS3(stream, s3AccessKey, s3SecretKey, s3Params, options, cb) {
  ...
  this.on("error", function (e) {
    ...
    if (self.stream) {
      self.streamErrorHandler &&
        self.stream.removeListener("error", self.streamErrorHandler);
      self.streamDataHandler &&
        self.stream.removeListener("data", self.streamDataHandler);
      self.streamEndHandler &&
        self.stream.removeListener("end", self.streamEndHandler);
    }

    if (self.uploadId) {
      var abortMultipartUploadParams = extendObj(
  ...
  if (!this.s3ObjectParams.Bucket || !this.s3ObjectParams.Key) {
    return this.emit(
      "error",
      new Error("Bucket and Key parameters for S3 Object are required!")
    );
  }
...
StreamingS3.prototype.begin = function () {
  ...
  this.streamEndHandler = function () {
    ...
    if (self.downloadStart) {
      self.stats.downloadTime = Math.round(
        (Date.now() - self.downloadStart) / 1000,
        3
      );
      self.stats.downloadSpeed = Math.round(
        self.totalBytes / (self.stats.downloadTime / 1000),
        2
      );
    }
    self.flushChunk();
  ...
  async.series(
    {
      createMultipartUpload: function (callback) {
        var createMultipartUploadParams = extendObj(
          self.s3Params,
          self.s3ObjectParams
        );
        self.s3Client.createMultipartUpload(
          ...
          function (err, data) {
            ...
            if (!data.UploadId)
              return callback(
                new Error(
                  "AWS SDK returned invalid object! Expecting UploadId."
                )
              );
    ...
    chunk.client.uploadPart(partS3Params, function (err, data) {
      if (err) {
        ...
        if (!data.ETag)
          return next(
            new Error(
              "AWS SDK returned invalid object when part uploaded! Expecting Etag."
            )
          );
    ...
    async.eachLimit(
      ...
      function (err) {
        ...
        if (
          ...
          !self.waiting &&
          !self.reading &&
          self.totalChunks === Object.keys(self.uploadedChunks).length
        ) {
          if (self.uploadStart) {
            self.stats.uploadTime = Math.round(
              (Date.now() - self.uploadStart) / 1000,
              3
            );
            self.stats.uploadSpeed = Math.round(
              self.totalBytes / (self.stats.uploadTime / 1000),
              2
            );
          }
          ...
          self.acknowledgeTimer = setTimeout(function () {
            self.finish();
          }, 500);
        }
      }
...
StreamingS3.prototype.finish = function () {
  ...
  var listPartsParams = extendObj(
    { UploadId: this.uploadId, MaxParts: this.totalChunks },
    this.s3ObjectParams
  );
  this.s3Client.listParts(listPartsParams, function (err, data) {
    if (err) return self.emit("error", err);

    if (!self.acknowledgeTimer)
      self.acknowledgeTimer = setTimeout(function () {
        self.finish();
      }, 1000);
    ...
    if (!data.Parts)
      return self.emit(
        "error",
        new Error("AWS SDK returned invalid object! Expecting Parts.")
      );
    ...
    for (var key in self.uploadedChunks) {
      completeMultipartUploadParams.MultipartUpload.Parts.push({
        ETag: self.uploadedChunks[key],
        PartNumber: key,
      });
    }

    completeMultipartUploadParams = extendObj(
    ...
    self.s3Client.completeMultipartUpload(
      ...
      function (err, data) {
        ...
        if (!data.ETag)
          return self.emit(
            "error",
            new Error("AWS SDK returned invalid object! Expecting file Etag.")
          );
        self.waitingTimer && clearTimeout(self.waitingTimer);
        self.acknowledgeTimer && clearTimeout(self.acknowledgeTimer);
    ...
    this.waitingTimer = setTimeout(function () {
      if (self.waiting && !self.finished) {
        return self.emit(
          "error",
          new Error(
            "AWS did not acknowledge all parts within specified timeout of " +
              self.options.waitTime / 1000 +
              " seconds."
          )
        );
      }
